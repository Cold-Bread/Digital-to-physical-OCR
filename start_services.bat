...
start "Main Service" cmd /k "cd %SCRIPT_DIR%backend2 && call %MAIN_PATH%\venvMain\Scripts\activate.bat && set PYTHONPATH=%SCRIPT_DIR%backend2 && python -m uvicorn main_app.main:app --reload --port 8000" Start all OCR services
...
set "TROCR_PATH=%SCRIPT_DIR%backend2\ocr_trocr_service"
...
:: Start TrOCR Service
echo Starting TrOCR Service on port 8002...
start "TrOCR Service" cmd /k "cd %TROCR_PATH% && call venvTrOCR310\Scripts\activate.bat && uvicorn app:app --reload --port 8002"

echo.
echo All services have been started!
echo Main Service: http://localhost:8000
echo PaddleOCR Service: http://localhost:8001
echo TrOCR Service: http://localhost:8002
echo.
pause
